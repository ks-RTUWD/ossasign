...
#
    ...
    gemstone_images = [
        pygame.image.load(os.path.join(image_path, "small_gold.png")).convert_alpha(),  # 작은 금
        pygame.image.load(os.path.join(image_path, "big_gold.png")).convert_alpha(),  # 큰 금
        pygame.image.load(os.path.join(image_path, "stone.png")).convert_alpha(),  # 돌
        pygame.image.load(os.path.join(image_path, "diamond.png")).convert_alpha()]  # 다이아몬드
    ...
    while running:
        ...
        if claw.offset.x < default_offset_x_claw:  # 원위치에 오면
            ...
            if caught_gemstone:  # 잡힌 보석이 있다면
                curr_score += caught_gemstone.price  # 점수 업데이트 처리 (나중에)
